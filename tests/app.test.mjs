...
test("styles.css 为珠子和白色串线提供实物质感层", () => {
  ...
  assert.match(css, /\.selected-bead\s*\{[^}]*box-shadow: 0 8px 14px rgb\(44 38 34 \/ 16%\);/s);
  assert.match(css, /\.selected-bead-core\s*\{[^}]*filter: saturate\(1\.14\) contrast\(1\.1\);/s);
  assert.match(css, /\.product-bead\.has-photo\s*\{[^}]*box-shadow: 0 5px 9px rgb\(44 38 34 \/ 12%\);/s);
  assert.match(css, /\.selected-bead-core\s*\{[^}]*background-size: 132%;/s);
...
test("createBraceletStateFromDesignPreset 复制到 DIY 时生成 20 颗完整手链", () => {
  getSortedDesignPresets().forEach((design) => {
    ...
    assert.equal(result.error, null);
    assert.equal(result.state.items.length, 20);
    assert.ok(coverageRatio >= 0.88);
    assert.ok(Math.max(...gapRatios) <= 1.1);
  });
});
...
test("styles.css 真实单珠图片圆形裁切且不重复拼接", () => {
  ...
  assert.match(css, /\.selected-bead-core, \.product-bead\s*\{[^}]*clip-path: circle\(44\.5% at 50% 50%\);/s);
...
// 验证珠子接近达标时尺寸平滑变大，不会在第 16 颗突然跳大。
test("calculateBeadPositions 在珠子达标前平滑放大", () => {
  ...
  const jumps = sizes.slice(1).map((size, index) => size - sizes[index]);

  assert.ok(sizes[0] > 48);
  assert.ok(sizes.at(-1) >= 84);
  assert.ok(Math.max(...jumps) <= 10);
});

// 验证按视频模板 16 颗时珠子已经能盖住串线，20 颗只是继续允许追加。
test("calculateBeadPositions 在 16 颗时珠子覆盖白色串线", () => {
  ...
  assert.ok(Math.min(...positions.map((position) => position.size)) >= 68);
  assert.ok(coverageRatio >= 0.9);
  assert.ok(Math.max(...gapRatios) <= 1.05);
...
test("calculateBraceletTrackFrame 与 calculateBeadPositions 使用同一条绳圈半径", () => {
  ...
  assert.equal(track.error, null);
  assert.equal(track.viewBoxRadius, 390);
